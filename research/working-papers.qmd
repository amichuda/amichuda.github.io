...
This paper illustrates and addresses weak identification concerns in the correlated random coefficient (CRC) model that Suri (2011) uses to study agricultural technology adoption.
Using the publicly available version of the dataset used in Suri (2011), cleaned per the author's instructions, we are unable to replicate the paper's main CRC model results.
...
This reveals that the key structural parameter in the CRC model ($\phi$) is prone to a weak identification problem.
We then propose a procedure to conduct weak-identification robust inference on $\phi$ using test inversion.
Only with this robust procedure accounting for weak identification are we able to reconcile the original Suri (2011) results.
:::

[PDF](https://drive.google.com/file/d/1CoQelaFIoGswFv4Kh_kMAIw42cjhuOEU/view?usp=sharing){.btn .btn-outline-primary .btn role="button"}

## Gendered Impacts of Reforming Night-Shift Work in India

[Link to Extended Abstract](https://drive.google.com/file/d/1SZofJ2SmoJabcdUJPwwzwBWB4Fvv5gAn/view?usp=sharing){.btn .btn-outline-primary .btn role="button"}
